...
df %>% gather(true_mean:ARMA31, key=type, value=SE) %>%
  ggplot(aes(x=trend_length, y=SE)) +
    geom_line(aes(color=type, linetype=type)) +
    scale_color_manual(values=c(2,3,1,1,1)) +
...
p <- df2 %>% gather(AR1_mean:ARMA31_lower, key=model, value=SE) %>%
  ggplot(aes(x=trend_length, y=SE)) +
    geom_line(aes(color=model, size=model)) +
    geom_line(y=1, linetype=2) +
    scale_color_manual(breaks=c("AR1_mean", "ARMA31_mean"), values=c(2,2,2,3,3,3),
                       labels=c("AR1", "ARMA31")) +
    scale_size_manual(values=rep(c(0.2,1.2,0.2),2), guide=FALSE) +
